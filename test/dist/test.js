...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof iterCeil10, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object which rounds each iterated value to the nearest power of 10 toward positive infinity', function test( t ) {
	...
	N = 101;
	it = iterCeil10( linspace( -200.0, 200.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( -200.0, 200.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = ceil10( x.next().value );
